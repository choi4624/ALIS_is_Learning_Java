...
public class Battle {
    ...
    void sixth(UnitHuman fight, UnitHuman.PlayerChara User, effect sound) throws InterruptedException, IOException {
        ...
        while (fight.MaxHP > 0) {
            ...
            System.out.println("공격하시겠습니까? 1. 예, 2. 아니오, 3. 적 상태 확인");
            if(status ==1) {
                System.out.println("스킬4: 지면강타");
                System.out.println("스킬5: 레일건 사격");
                System.out.println("스킬6: 코어 저격");
                System.out.println("스킬7: 장갑 파괴");
                System.out.println("스킬8: 시스템 해킹");
                System.out.println("스킬9: 밸런스붕괴");
            }
            Scanner typing = new Scanner(System.in);
            String console = typing.next();
            ...
            switch (console){
                case "1" -> {
                    ...
                    dealt = atk.BasicAttack(fight, User);
                    System.out.println(dealt+" 대미지 ");
                    if(dealt < 200)
                        System.out.println("이건 뭔가 잘못됐어.");
                }
                ...
                case "5" -> {
                    skill.RailGunShoot(fight, User);
                }
                case "6" -> {
                    skill.coreATK(fight,User);
                }
                case "7" -> {
                    skill.ArmorBreak(fight,User);
                }
                case "8" -> {
                    skill.SystemHack(fight,User);
                }
                case "9" -> {
                    skill.statup(User);
                    startHP = User.MaxHP;
            ...
            if(trun == 0)
            {dealt = enemyatk.EnemyAttack(fight,User);
                System.out.println(" 무기 "+fight.weapon + " / 적의 피해 "+dealt);
                trun = 1;}
            else
            {   enemyatk.powerup(fight,User);
                trun =0;}


            System.out.println(User.name + "의 현재체력" + User.MaxHP);

            if (hacking == 1 && count > 0)
                count -= 1;
            if (count == 0)
                hacking = 0;
            if (hacking ==0)
                skill.ErrorRestore(fight);
    ...
    public void blacksuit4(UnitHuman fight, UnitHuman.PlayerChara User, effect sound) throws InterruptedException {
        int startHP = User.MaxHP;
        int startMP = User.MaxMP;
        int status = 0;
        while (fight.MaxHP > 0) {
